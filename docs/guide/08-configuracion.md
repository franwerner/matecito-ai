...
> **Compatibilidad:** un `config.json` plano de antes de multi-dominio (con `models` / `strictTdd` en la raíz) se **migra automáticamente** a `domainConfig.development` al leerlo —no hace falta tocar nada— y se persiste en el formato nuevo en el próximo guardado.
...
La captura de decisiones (in-flow en development, mine gate post-verify en design) no es config: corre
siempre, sin interruptor. Ver [05](05-auto-mine.md).
