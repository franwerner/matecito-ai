...
| **Decisiones implícitas** | las convenciones viven en la cabeza del autor; el agente las reinventa o las viola | **ADRs** — decisiones capturadas y respetadas _(el decision record de development; en design es DDR)_ |
...
- **Capa de decisiones** — ADRs + las skills `bootstrap` (capturar), `validate` (chequear) y `mine` (descubrir desde código). El concepto de decision record es del núcleo; el tipo concreto (ADR) es de development. Ver [04](04-decisiones-adr.md).
...
development es **uno** de los dominios del ecosistema; convive con otros (`design`, …) bajo el mismo núcleo. Agregar un dominio nuevo es **implementar el contrato de área** —`manifest.json` + un fragmento `CLAUDE.md`— sin tocar el núcleo (ver [cómo se arma un dominio](../../payload/domains/README.md)). Por eso esta guía habla de SDD, ADR y codegraph: son el *binding* de development, no el ecosistema entero.
