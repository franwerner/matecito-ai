...
3. Project-level conventions live in the project's own files — read `.claude/adr/` (architecture decisions), `CLAUDE.md`, and any `config.yaml` the phase references. Those are the project's standards in matecito-ai; there is no separate registry to consult.
