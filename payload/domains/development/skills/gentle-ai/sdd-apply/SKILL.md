...
1. Reposition your worktree onto `base`. First read your own branch name from inside the worktree:
   ...
   your own, typically `worktree-agent-<hex>`) — **never** the working branch's own name (e.g. `main`),
   even if you know it: worktrees share the ref store, and `git checkout -B <working-branch> <base>` run
...
- **The ratified shape returns to you ONLY in a later re-dispatch's own instructions.** Once ratified (or user-adjusted), you write it into the **code** — where this domain's rule already says a contract's shape lives; no store is invented for it. Never guess it and never read `apply-progress` looking for it: if you are re-dispatched to the point a contract governs and your instructions carry no shape for it, return `blocked` naming the missing contract instead. See `~/.claude/matecito-ai/domains/development.md`, "Forwarding a ratified contract shape to the proposing phase."
...
- **Repositioning onto `base`, then the base handshake, both run before any write, in Isolated Run Mode.** The harness's worktree starting point is not the working branch's base (it can be `origin/<branch>`, behind the working branch's local `HEAD`) — reposition via `git checkout -B <your-branch> <base>`, where `<your-branch>` is read from inside the worktree (`git rev-parse --abbrev-ref HEAD`) and is **never the working branch's own name** (e.g. `main`): worktrees share the ref store, so resetting that branch from inside a worktree moves it everywhere and orphans commits. A failed repositioning, or an unverified base after it (`HEAD != base`, or a dirty `git status --porcelain`), both mean implementing nothing — report `not-implemented / base-not-established` instead of guessing at what the worktree actually contains. See Step 3b and `parallel-batch.md` → "Repositioning onto `base`" and "The base handshake"
