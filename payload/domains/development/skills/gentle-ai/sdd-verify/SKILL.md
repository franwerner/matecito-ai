...
6e. Validate the durable store's STRUCTURE — **only when this change declares New or Modified capabilities**; otherwise skip entirely, with no mention. Since `sdd-apply`'s Step 5b already folded this change's delta before verify runs, an on-disk file is now EXPECTED for every capability this change declares New or Modified — not "if a durable spec exists": a declared capability with no file on disk is itself a finding (the fold did not happen, or happened somewhere the delta did not intend), not a silent skip. Run the mechanical engine rather than re-deriving its checks by hand:
