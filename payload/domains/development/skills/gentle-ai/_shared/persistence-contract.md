...
<!-- matecito-ai: el registry/inyección fue removido. Las fases cargan su propia skill directamente y leen las convenciones del proyecto desde sus archivos (.matecito-ai/edr/, CLAUDE.md, config.yaml). -->

Each SDD phase loads its own `SKILL.md` (plus any module it explicitly references). Project conventions are read directly from the project's files — `.matecito-ai/edr/` for architecture decisions, `CLAUDE.md`, and `config.yaml`. There is no skill registry or pre-injected `Project Standards` block in matecito-ai. See Section A of `sdd-phase-common.md`.
