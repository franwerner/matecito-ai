...
2. **Mostrá un resumen** agrupado por tipo y status (`Inferred` pendiente de ratificar, `Accepted`, `Draft` con qué falta, `Deprecated`). Si `repo.components` está declarado, sumá al resumen los specs **sin** línea `Components:` (`development-spec-validate` los reporta como WARNING).
3. **Preguntá si algún `Draft` está listo para completarse, o algún `Inferred` heredado está listo para ratificar** — es lo más importante del update; sin esto los "lo definimos después" y los candidatos sin revisar se pierden.
4. **Después preguntá qué más:**
   - **Completar un `Draft`** → recorrer sus secciones faltantes, `Status → Accepted`.
   - **Ratificar un `Inferred`** → distinto de completar un `Draft`: el `Inferred` arrancó con secciones pre-cargadas desde evidencia as-built, no vacío. Revisá cada sección contra el comportamiento real (¿es la intención, o calcó un bug?), corregí lo que no sea la intención, escrubeá cualquier identificador interno volátil que haya quedado (ver vocabulario), agregá los escenarios/bordes que falten, y recién entonces `Status → Accepted`.
   ...
   - **Asignar el componente de un spec que quedó sin `Components:`** → uno a uno (nunca en bulk sin revisar): mostrá la ruta o el comportamiento descrito, preguntá qué superficie(s) del set le corresponden, y escribí la línea.
