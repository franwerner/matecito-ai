...
Still show the summary table and the provenance line. Note: *"Could not render sample (MCP unavailable). Save anyway on your OK."* Do not block.
...
1. **Read the image.** Use the agent's vision input — the same capability the main workflow's self-check uses to inspect the live preview.
