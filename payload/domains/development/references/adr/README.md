...
Referencia canónica del **concepto** de ADR (Architecture Decision Record). Es la fuente de verdad de la *idea*; cualquier skill o agente que trabaje con ADRs apunta acá en vez de redefinirla. La *estructura/plantilla* concreta se define por separado; esto define qué cuenta como ADR y qué no.
...
- **No es una unidad de trabajo.** Una tarea o paso de implementación se ejecuta y se termina; no decide nada. La mayoría del trabajo es mecánico y no merece ADR. Forzar un ADR por cada cosa hecha es ruido.
...
- **Aceptado** — una persona lo ratificó y completó el porqué (contexto, alternativas, consecuencias). Recién ahí es un ADR pleno.
