...
- Figma: the `figma` MCP registered AND a file reachable (a file read succeeds).
...
Detection notes:
- `figmaConnected`: the `figma` MCP must be registered AND a file reachable (a file read succeeds). If the MCP is not registered or no file is connected at init time, detected as ❌ even if a file exists in the user's Figma account. Document the limitation: Figma not connected at init → detected absent.
