...
description: >
  ...
  brief-intake: ask targeted questions, classify the work, and catch DDR conflicts or undecided brand
  questions before exploration begins.
---
...
4. Early guard (DDR activation gate): if `.matecito-ai/ddr/` is absent or empty, DDRs are inactive —
...
5. Produce the structured brief-intake artifact and return it.
...
Explain the WHY behind each classification call in 1-2 lines — the design principle at stake. Cite
the `design-principles` catalog rather than improvising; when a concept the person may not know
surfaces, derive to `explain-concept`. Do not paraphrase the full mentor rule (it lives in the domain
CLAUDE.md).
