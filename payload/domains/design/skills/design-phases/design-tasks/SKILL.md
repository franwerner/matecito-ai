...
5. Per task, add a verifiable `criteria:` sub-line (mandatory — consumed by `design-verify`). When the
   task touches a brand decision, emit a concern-mapped `· ddr: <surface>/<slug>` ref, whether or not
   the DDR exists yet — this is decision-gap detection, always on, and does NOT depend on DDRs
   existing. Then, for each `· ddr:`, check whether `.matecito-ai/ddr/<surface>/<slug>.md` exists: if
   NOT, the ref is a dangling decision gap — leave it verbatim (do not modify or mark it). With zero
   DDRs, every brand decision is a gap (bootstrap the first ones). The dangling refs in the artifact
   are the gap list that `design-verify` consumes. Keep the `- [ ]` checkbox so `design-produce` can
   mark progress.
