...
Always on (does NOT depend on DDRs existing). Read the tasks artifact; collect all `· ddr: <surface>/<slug>` whose file `.matecito-ai/ddr/<surface>/<slug>.md` does NOT exist — these are the decision gaps. For each gap: (a) check the task is `[x]` (the piece was produced); (b) check its `criteria:` passes in the produced work (Figma inspection or the produced asset). If (a) and (b) → `implemented: yes`; otherwise `implemented: no`. Add a `## Decision Gaps` section to the verify-report with the table `| surface/slug | task | implemented? |`. If the section has at least one `yes`, the orchestrator may trigger the mine gate post-verify (executor: design-decisions-mine). -->
7c. (Decision-gap confirmation) From the tasks artifact collect all `· ddr: <surface>/<slug>` refs whose target file does NOT exist → these are decision gaps. For each: confirm task is `[x]` AND `criteria:` passes in the produced work → mark `implemented: yes/no`. Add `## Decision Gaps` to the verify-report: `| surface/slug | task | implemented? |`.
