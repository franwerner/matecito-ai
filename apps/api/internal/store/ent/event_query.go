...
func (_q *EventQuery) QueryChange() *ChangeQuery {
...
func (_q *EventQuery) WithChange(opts ...func(*ChangeQuery)) *EventQuery {
	query := (&ChangeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := _q.withChange; query != nil {
		if err := _q.loadChange(ctx, query, nodes, nil,
			func(n *Event, e *Change) { n.Edges.Change = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadChange(ctx context.Context, query *ChangeQuery, nodes []*Event, init func(*Event), assign func(*Event, *Change)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Event)
	for i := range nodes {
		fk := nodes[i].ChangeID
...
func (_q *EventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withChange != nil {
			_spec.Node.AddColumnOnce(event.FieldChangeID)
		}
	}
