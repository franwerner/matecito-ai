...
- **Gates bloqueantes:** diferidos. El primer slice de la superficie es de solo lectura; los gates que retienen una request abierta hasta que la UI los resuelve se incorporan en la fase de escritura posterior (el mecanismo previsto aprovecha que streamable HTTP mantiene la request abierta, con rendezvous por un canal interno del daemon).
...
- Trade-off: diferir los gates bloqueantes deja el primer slice sin control de flujo interactivo; el mecanismo de retención de request queda pendiente para la fase de escritura.
