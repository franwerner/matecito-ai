...
El comportamiento del CLI de matecito-ai —qué hace el sistema ante cada situación— está definido a nivel de proyecto en `.matecito-ai/development-specs/`, **organizado por tipo** (`flow` / `rule` / `lifecycle` / `process`). Vive en el root porque el comportamiento es intencionalmente multi-superficie y separarlo por componente escondería esa relación (`rule/single-rooted-spec-store.md`) — no porque el producto tenga dos mitades.
...
Los specs dicen *qué hace* el sistema; el *por qué* de cada elección técnica vive en el store de EDRs (`.matecito-ai/edr/`) y el *cómo* literal en el código. Para definir, actualizar o validar comportamiento, usá `development-spec-bootstrap` (y `development-spec-validate` para chequear coherencia).

## Decisiones de ingeniería (EDRs)

Las decisiones técnicas viven en un solo store, `.matecito-ai/edr/`, organizado por dominio (`structure` / `contracts` / `tech`), y gobiernan todo el repo, incluido el CLI. Antes de tocar arquitectura, capas, errores, datos, transporte o convenciones, leé `.matecito-ai/edr/INDEX.md` primero y después el índice del dominio correspondiente.
