...
func tempDir(t *testing.T) string {
	t.Helper()
	d, err := os.MkdirTemp("", "proofshot-check-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
// TestDetectCLI mirrors the five spec scenarios of
...
// proofshot matrix (task 3.5), so a divergence between the two surfaces fails
// on exactly one side.
func TestDetectCLI(t *testing.T) {
	...
	t.Run("canonical-present", func(t *testing.T) {
		d := tempDir(t)
		writeBin(t, d, "proofshot", 0, "1.0.0")
		path := filepath.Join(d, "proofshot")
	...
	t.Run("canonical-absent", func(t *testing.T) {
		d := tempDir(t)
		path := filepath.Join(d, "proofshot") // never written
		resolveBinary = func() (string, error) { return path, nil }
	...
	t.Run("real-resolver", func(t *testing.T) {
		...
		resolveBinary = install.ProofshotBinaryPath
