...
#### Scenario: No explicit request means inactive and silent

- GIVEN a request that says nothing about isolation
- WHEN the change runs
- THEN isolation is inactive, no workspace is opened, and nothing about it is mentioned

#### Scenario: One workspace per change

- GIVEN a change whose workspace is already open
- WHEN a later phase runs or the session resumes
- THEN that same workspace is used and no second one is opened
